...
#include "imguiext/SelectionWidget.h"
...
int main(int argc, char* argv[]) {
    ...
    igl::read_triangle_mesh(
        argc > 1 ? argv[1] : "C:/Users/arya/Documents/Model/mesh/MachinedPart2.STL", V, F);

    // Plot the mesh
    igl::opengl::glfw::Viewer vr;
    imguiext::ImGuiPlugin imgui_plugin;
    vr.plugins.push_back(&imgui_plugin);
    imguiext::SelectionWidget widget;
    ...
    tree.init(V, F);
    widget.callback = [&]() {
        screen_space_selection(V, F, tree, vr.core().view, vr.core().proj, vr.core().viewport, widget.L, W,
                               and_visible);
        update();
    };
    vr.callback_key_pressed = [&](decltype(vr)&, unsigned int key, int mod) {
        switch (key) {
            case ' ': only_visible = !only_visible;
                update();
                return true;
            case 'D':
            case 'd': W.setZero();
